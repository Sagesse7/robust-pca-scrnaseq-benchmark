# =========================================================
# Real-data benchmark for Bhattacherjee GSE124952
...
EXPR_PATH <- file.path(DATA_ROOT, "bhattacherjee", "GSE124952_expression_matrix.csv.gz")
META_PATH <- file.path(DATA_ROOT, "bhattacherjee", "GSE124952_meta_data.csv.gz")

CELL_ID_COL <- "V1"
LABEL_COL   <- "CellType"       # Options include "CellType" (8 classes) or "L2_clusters" (104 classes)

run_id <- paste0("run_", format(Sys.time(), "%Y%m%d_%H%M%S"))
OUT_DIR <- if (is.null(cli[["output-dir"]])) file.path(PROJECT_DIR, "results", "bhattacherjee", run_id) else cli[["output-dir"]]
...
SUBSET_N    <- if (is.null(cli[["subset-n"]])) 10000L else as.integer(cli[["subset-n"]])
...
read_real_csv_bhattacherjee <- function(expr_path, meta_path, label_col = "CellType",
                                        cell_id_col = "V1", reference_n = NA_integer_,
                                        seed = 1L) {
  meta <- data.table::fread(meta_path)
  
  if (!(cell_id_col %in% colnames(meta))) {
    stop(sprintf("Column '%s' not found in metadata.", cell_id_col))
  }
  if (!(label_col %in% colnames(meta))) {
    stop(sprintf("Column '%s' not found in metadata.", label_col))
  }
  
  meta[[cell_id_col]] <- as.character(meta[[cell_id_col]])
  meta[[label_col]] <- as.character(meta[[label_col]])
  meta <- meta[!is.na(get(cell_id_col)) & get(cell_id_col) != "" &
                 !is.na(get(label_col)) & get(label_col) != ""]
  
  header <- names(data.table::fread(expr_path, nrows = 0L))
  gene_col <- header[1]
  expr_cell_ids <- header[-1]
  
  meta <- meta[get(cell_id_col) %in% expr_cell_ids]
  if (nrow(meta) < 3L) {
    stop("Too few metadata cells matched expression matrix columns.")
  }
  
  ref_rows <- random_sample_rows(n_total = nrow(meta), n = reference_n, seed = seed)
  meta_ref <- meta[ref_rows]
  selected_cells <- meta_ref[[cell_id_col]]
  
  select_cols <- c(gene_col, selected_cells)
  expr_dt <- data.table::fread(expr_path, select = select_cols)
  
  gene_names <- expr_dt[[gene_col]]
  expr_mat <- as.matrix(expr_dt[, ..selected_cells])
  storage.mode(expr_mat) <- "numeric"
  rownames(expr_mat) <- make.unique(as.character(gene_names))
  
  X <- t(expr_mat)  # cells x genes
  rownames(X) <- selected_cells
  
  labels <- meta_ref[[label_col]]
  ...
  list(
    ...
    metadata = meta_ref,
    total_cells_available = nrow(meta),
    reference_n = nrow(X)
  )
}
...
# Main
# -----------------------------
required_bhatt_files <- c(EXPR_PATH, META_PATH)
if (any(!file.exists(required_bhatt_files))) stop("Bhattacherjee data files not found: ", paste(required_bhatt_files[!file.exists(required_bhatt_files)], collapse = ", "))
...
write.csv(data.frame(
  code_version = "20260712_public_release_candidate", dataset = "Bhattacherjee", hvg = TOP_HVG, subset_n = SUBSET_N, repeats = REPS,
...
if (!dir.exists(PARTIAL_DIR)) dir.create(PARTIAL_DIR, recursive = TRUE, showWarnings = FALSE)

set.seed(BASE_SEED)
methods_all <- build_methods(MAX_PC)
methods <- methods_all[METHOD_NAMES]

cat("Reading Bhattacherjee GSE124952 data...\n")
obj <- read_real_csv_bhattacherjee(
  expr_path = EXPR_PATH,
  meta_path = META_PATH,
  label_col = LABEL_COL,
...
cat(sprintf("Available QC-passed cells matched to expression matrix: %d\n",
...
rep_results <- future_lapply(
  ...
  FUN = function(rep) {
    ...
    write.csv(
      rep_df,
      file.path(PARTIAL_DIR, sprintf("rep_%02d.csv", rep)),
      row.names = FALSE
    )
    rep_df
  },
