# =========================================================
# Real-data benchmark for PBMCs
# Full-cell reference + repeated random subsets
...
suppressPackageStartupMessages({
  ...
  library(rpca)        # PCP
  library(data.table)  # fast gz CSV I/O
  library(Matrix)      # 10x MatrixMarket I/O
...
PBMC_DIR <- file.path(DATA_ROOT, "pbmcs")
MTX_PATH <- file.path(PBMC_DIR, "filtered_feature_bc_matrix", "matrix.mtx")
FEATURE_PATH <- file.path(PBMC_DIR, "filtered_feature_bc_matrix", "features.tsv")
BARCODE_PATH <- file.path(PBMC_DIR, "filtered_feature_bc_matrix", "barcodes.tsv")
CLUSTER_PATH <- file.path(PBMC_DIR, "clusters.csv")

CELL_ID_COL <- "Barcode"
LABEL_COL   <- "Cluster"
REQUIRED_FEATURE_TYPE <- "Gene Expression"

run_id <- paste0("run_", format(Sys.time(), "%Y%m%d_%H%M%S"))
OUT_DIR <- if (is.null(cli[["output-dir"]])) file.path(PROJECT_DIR, "results", "pbmcs", run_id) else cli[["output-dir"]]
OUT_CSV <- file.path(OUT_DIR, "realdata_full_vs_subset_results_with_pc_similarity.csv")

REFERENCE_N <- NA_integer_      # NA uses all QC-passed cells matched to the expression matrix
SUBSET_N    <- if (is.null(cli[["subset-n"]])) 400L else as.integer(cli[["subset-n"]])
...
# -----------------------------
# Read PBMCs 10x matrix + cluster labels
# -----------------------------
random_sample_rows <- function(n_total, n, seed = 1L) {
  if (is.na(n) || is.null(n)) return(seq_len(n_total))
  n <- min(as.integer(n), n_total)
  if (n >= n_total) return(seq_len(n_total))

  set.seed(seed)
  sort(sample(seq_len(n_total), size = n, replace = FALSE))
}

read_real_pbmc_10x <- function(mtx_path, feature_path, barcode_path, cluster_path,
                               label_col = "Cluster", cell_id_col = "Barcode",
                               reference_n = NA_integer_, seed = 1L,
                               required_feature_type = "Gene Expression") {
  clusters <- data.table::fread(cluster_path)
  
  if (!(cell_id_col %in% colnames(clusters))) {
    stop(sprintf("Column '%s' not found in cluster file.", cell_id_col))
  }
  if (!(label_col %in% colnames(clusters))) {
    stop(sprintf("Column '%s' not found in cluster file.", label_col))
  }
  
  features <- data.table::fread(feature_path, header = FALSE)
  barcodes <- data.table::fread(barcode_path, header = FALSE)[[1]]
  barcodes <- as.character(barcodes)
  
  X_genes_cells <- Matrix::readMM(mtx_path)
  if (nrow(X_genes_cells) != nrow(features)) {
    stop("Number of matrix rows does not match features.tsv.")
  }
  if (ncol(X_genes_cells) != length(barcodes)) {
    stop("Number of matrix columns does not match barcodes.tsv.")
  }

  if (ncol(features) < 3L) {
    stop("features.tsv has no third feature_type column; RNA-only filtering cannot be verified.")
  }
  feature_types_input <- trimws(as.character(features[[3]]))
  keep_feature_type <- !is.na(feature_types_input) & feature_types_input == required_feature_type
  n_features_input <- nrow(features)
  n_features_retained <- sum(keep_feature_type)
  if (n_features_retained < 1L) {
    stop("No features matched required feature_type: ", required_feature_type)
  }

  # Filter raw matrix rows before library-size normalization, log1p, HVG
  # selection, and centering/scaling. This keeps the PBMC analysis RNA-only.
  X_genes_cells <- X_genes_cells[keep_feature_type, , drop = FALSE]
  features <- features[keep_feature_type]
  feature_types <- feature_types_input[keep_feature_type]
  
  gene_names <- as.character(features[[2]])
  if (any(is.na(gene_names) | gene_names == "")) {
    gene_names <- as.character(features[[1]])
  }
  gene_names <- make.unique(gene_names)
  
  clusters[[cell_id_col]] <- as.character(clusters[[cell_id_col]])
  clusters[[label_col]] <- as.character(clusters[[label_col]])
  clusters <- clusters[!is.na(get(cell_id_col)) & get(cell_id_col) != "" &
                         !is.na(get(label_col)) & get(label_col) != ""]
  
  barcode_index <- match(clusters[[cell_id_col]], barcodes)
  clusters <- clusters[!is.na(barcode_index)]
  barcode_index <- barcode_index[!is.na(barcode_index)]
  if (nrow(clusters) < 3L) {
    stop("Too few clustered cells matched barcodes.tsv.")
  }
  
  ref_rows <- random_sample_rows(n_total = nrow(clusters), n = reference_n, seed = seed)
  clusters_ref <- clusters[ref_rows]
  selected_col_idx <- barcode_index[ref_rows]
  selected_cells <- clusters_ref[[cell_id_col]]
  
  X <- t(as.matrix(X_genes_cells[, selected_col_idx, drop = FALSE]))
  storage.mode(X) <- "numeric"
  rownames(X) <- selected_cells
  colnames(X) <- gene_names
  
  labels <- clusters_ref[[label_col]]
  names(labels) <- selected_cells
  ...
  list(
    ...
    required_feature_type = required_feature_type,
    n_features_input = n_features_input,
    n_features_retained = n_features_retained,
    n_features_excluded = n_features_input - n_features_retained,
    metadata = clusters_ref,
    total_cells_available = nrow(clusters),
    reference_n = nrow(X)
  )
}
...
# Main
# -----------------------------
required_pbmc_files <- c(MTX_PATH, FEATURE_PATH, BARCODE_PATH, CLUSTER_PATH)
if (any(!file.exists(required_pbmc_files))) stop("PBMC data files not found: ", paste(required_pbmc_files[!file.exists(required_pbmc_files)], collapse = ", "))
...
write.csv(data.frame(
  code_version = "20260729_public_pcp_xv", dataset = "PBMCs", hvg = TOP_HVG, subset_n = SUBSET_N, repeats = REPS,
  ...
  pbmc_feature_filter = "Gene Expression before normalization and HVG selection",
...
cat("Reading PBMCs 10x data...\n")
obj <- read_real_pbmc_10x(
  mtx_path = MTX_PATH,
...
cat(sprintf("Available clustered cells matched to expression matrix: %d\n",
            obj$total_cells_available))
cat(sprintf("Reference data read for benchmark: %d cells x %d genes\n",
            nrow(counts_raw), ncol(counts_raw)))
cat(sprintf(
  "[FEATURE_TYPE] retained %d/%d raw features with feature_type='%s'; excluded=%d\n",
  obj$n_features_retained, obj$n_features_input, obj$required_feature_type,
  obj$n_features_excluded
))
...
selected_index <- match(prep$selected_genes, obj$genes)
if (anyNA(selected_index)) stop("Selected HVG names could not be mapped to retained raw features.")
selected_feature_types <- obj$feature_types[selected_index]
if (any(selected_feature_types != REQUIRED_FEATURE_TYPE)) {
  stop("A non-Gene Expression feature remained after RNA-only preprocessing.")
}
selected_features <- data.frame(
  Rank = seq_along(prep$selected_genes),
  FeatureName = prep$selected_genes,
  FeatureType = selected_feature_types,
  stringsAsFactors = FALSE
)
write.csv(selected_features, file.path(OUT_DIR, "selected_hvg_features.csv"), row.names = FALSE)
write.csv(data.frame(
  required_feature_type = REQUIRED_FEATURE_TYPE,
  features_input_total = obj$n_features_input,
  features_retained = obj$n_features_retained,
  features_excluded = obj$n_features_excluded,
  selected_hvg = nrow(selected_features),
  selected_non_gene_expression = sum(selected_feature_types != REQUIRED_FEATURE_TYPE),
  filtering_stage = "raw_counts_before_library_normalization_log1p_hvg_scaling",
  stringsAsFactors = FALSE
), file.path(OUT_DIR, "feature_filter_manifest.csv"), row.names = FALSE)
